...
async function findComponents(options) {
	...
	const templates = await findFiles(pattern, {cwd: config.componentDir, ignore: config.ignore.concat(["**/*--*.*"])});
	for (const file of templates) {
		...
		const configFiles = await findFiles(`${path.dirname(file)}/${handle}.config.{yml,yaml}`, {cwd: config.componentDir});
			...
			if (!!compConfig === true) {
				...
				if (compConfig.variants) {
					for (const variant of compConfig.variants) {
						...
						const vtempls = await findFiles(`${path.dirname(file)}/${handle}--${variant.name}.{${config.templateExtensions}}`, {cwd: config.componentDir}); 
						if (vtempls.length > 0) {
